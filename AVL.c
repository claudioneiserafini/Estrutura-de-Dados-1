...
void empilharEstatistica(Estatistica **topo, char jogador[],
                         char tipoEstatistica[]) {
...
void desfazerUltimoEvento(Evento **listaEventos,
  ...
  while (temp->proximo != NULL) {
    if (strcmp(temp->proximo->jogador, ultimaEstatistica->jogador) == 0 &&
        strcmp(temp->proximo->tipoEvento, ultimaEstatistica->tipoEstatistica) ==
            0) {
      anterior = temp;
    }
    temp = temp->proximo;
...
void enfileirarSubstituicao(FilaSubstituicoes *fila, int time,
                            int jogadorEntrando, int jogadorSaindo) {
...
Substituicao *realizarSubstituicao(FilaSubstituicoes *fila, Time times[2],
                                   Evento **listaEventos) {
  if (fila->inicio == NULL) {
    return NULL;
  }
  ...
  strcpy(times[timeIndex].jogadoresAtivos[saindoIndex],
         times[timeIndex].reservas[entrandoIndex]);
  ...
  *listaEventos = inserirEvento(
      *listaEventos, times[timeIndex].jogadoresAtivos[saindoIndex], "entrou");
...
int main() {
  ...
  do {
    ...
    case 2: {
      ...
      enfileirarSubstituicao(&filaSubstituicoes, timeIndex,
                             jogadorEntrandoIndex, jogadorSaindoIndex);
